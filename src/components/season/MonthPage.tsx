...
import { getMonthBySlug, monthStrip } from '@/data/barcelona-months';
...
// Several stored tour titles carry raw HTML entities from the original scrape.
// Decoding them here keeps the JSON-LD clean without editing tours.ts, which
// would change the existing tour pages' own titles.
...
export function buildMonthPage(slug: string) {
  ...
  const climate = barcelonaClimate[data.monthKey];
  ...
  const articleSchema = {
    ...
    '@type': 'Article',
    headline: `Barcelona in ${data.month}: Weather, Events and the Best Tours`,
  ...
  const itemListSchema = {
    '@context': 'https://schema.org',
    '@type': 'ItemList',
    name: `The best tours in Barcelona in ${data.month}`,
    ...
    itemListElement: picks.map(({ tour }, i) => ({
      ...
      item: {
        '@type': 'Product',
        name: decodeEntities(tour.title),
    ...
    <>
      ...
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-8">
        <Breadcrumbs
          items={[
            { label: 'Home', href: '/' },
            { label: 'Best time to visit Barcelona', href: HUB_PATH },
            { label: `Barcelona in ${data.month}` },
          ]}
        />
        ...
        <article>
          <header>
            <h1 className="text-3xl sm:text-4xl font-bold text-gray-900 leading-tight">
              Barcelona in {data.month}: Weather, Events and the Best Tours
            </h1>
          ...
          <section aria-labelledby="quick-pick" className="mt-8">
            <h2 id="quick-pick" className="text-xl font-bold text-gray-900">
              Quick pick: the best Barcelona tours in {data.month}
            </h2>
            <div className="mt-3 overflow-x-auto rounded-lg border border-gray-200">
              <table className="w-full min-w-[640px] text-left text-sm">
                <caption className="sr-only">
                  The ten best Barcelona tours in {data.month}, ranked
                </caption>
            ...
            <p className="mt-2 text-xs text-gray-500">
              Prices, ratings and review counts verified against live GetYourGuide listings on 28 July 2026.
              Duration varies by the option you choose at checkout.
            </p>
          </section>

          {/* 1. Verdict */}
          <H2 id="is-it-good">Is {data.month} a good time to visit Barcelona?</H2>
          ...
          {/* 2. Weather */}
          <H2 id="weather">Barcelona weather in {data.month}</H2>
          <Capsule>{data.weather.capsule}</Capsule>
          <div className="mt-4 overflow-x-auto rounded-lg border border-gray-200">
            <table className="w-full min-w-[560px] text-left text-sm">
              <caption className="sr-only">Barcelona climate averages for {data.month}</caption>
              <thead className="bg-gray-50 text-gray-700">
                <tr>
              ...
              <tbody className="divide-y divide-gray-100">
                <tr>
                  ...
                  <td className="px-3 py-2.5 text-gray-600">Ranges from 13.5C in January to 28.7C in July</td>
                </tr>
                <tr>
                  ...
                  <td className="px-3 py-2.5 text-gray-600">Ranges from 4.7C in January to 21.5C in July and August</td>
                </tr>
                <tr className="bg-amber-50/60">
                  <th scope="row" className="px-3 py-2.5 font-medium text-gray-900">Sea temperature</th>
                  <td className="px-3 py-2.5 text-gray-900 font-semibold">{climate.seaTempC}C</td>
                  <td className="px-3 py-2.5 text-gray-600">
                    Barcelona&apos;s biggest seasonal swing: 25 to 26C in August against 13C in February
                  </td>
                </tr>
                <tr>
                  <th scope="row" className="px-3 py-2.5 font-medium text-gray-900">Rain</th>
                  <td className="px-3 py-2.5 text-gray-800">{climate.wetDays} wet days, {climate.rainMm}mm</td>
                  <td className="px-3 py-2.5 text-gray-600">Wettest is October at 105mm, driest is December at 25mm</td>
                </tr>
                <tr>
                  ...
                  <td className="px-3 py-2.5 text-gray-600">From 9h 13m in December to 15h 10m in June</td>
                </tr>
              </tbody>
            </table>
          </div>
          <p className="mt-2 text-xs text-gray-500">
            Air temperature, rainfall and wet days are a ten-year observed mean for Barcelona from the{' '}
            ...
            temperature is the Mediterranean immediately off Barcelona, given as a range rather than a
            single mean because the swing inside a month is large in spring and autumn.
          </p>
          ...
          <p className="mt-2 text-xs text-gray-500">
            Tourist tax rates:{' '}
            <a href={TOURIST_TAX_SOURCE.url} target="_blank" rel="noopener noreferrer nofollow" className="underline">
              {TOURIST_TAX_SOURCE.label}
            </a>
            . Park Güell prices and timed-entry rules:{' '}
            <a href={PARK_GUELL_SOURCE.url} target="_blank" rel="noopener noreferrer nofollow" className="underline">
              {PARK_GUELL_SOURCE.label}
            </a>
            . The tourist tax is quoted in euros because Barcelona sets it in euros; tour prices on this
            site are in pounds.
          </p>

          {/* 4. Tours */}
          <H2 id="best-tours">The best tours in Barcelona in {data.month}</H2>
          ...
          <div className="mt-4 overflow-x-auto rounded-lg border border-gray-200">
            <table className="w-full min-w-[640px] text-left text-sm">
              <caption className="sr-only">Dated events in Barcelona in {data.month} {year}</caption>
              <thead className="bg-gray-50 text-gray-700">
                <tr>
          ...
          <div className="mt-4 overflow-x-auto rounded-lg border border-gray-200">
            <table className="w-full min-w-[640px] text-left text-sm">
              <caption className="sr-only">Closures affecting Barcelona sights in {data.month}</caption>
              <thead className="bg-gray-50 text-gray-700">
                <tr>
          ...
          <p className="mt-2 text-xs text-gray-500">
            Opening hours, prices and closure dates come from each attraction&apos;s own timetable:{' '}
            <a href={SAGRADA_SOURCE.url} target="_blank" rel="noopener noreferrer nofollow" className="underline">
              {SAGRADA_SOURCE.label}
            </a>
            ,{' '}
            <a href={PEDRERA_SOURCE.url} target="_blank" rel="noopener noreferrer nofollow" className="underline">
              {PEDRERA_SOURCE.label}
            </a>{' '}
            and{' '}
            <a href={PARK_GUELL_SOURCE.url} target="_blank" rel="noopener noreferrer nofollow" className="underline">
              {PARK_GUELL_SOURCE.label}
            </a>
            . Public holidays were checked against the official Catalan calendar.
          </p>
          ...
          {/* 7. Packing */}
          <H2 id="what-to-pack">What to wear and pack for Barcelona in {data.month}</H2>
          ...
          <Capsule>
            We start from verified GetYourGuide review volume, then re-order for the month so that the
            experiences that actually work in {data.month}&apos;s weather, sea temperature and daylight sit
            highest. Every price, rating and review count on this page was checked against the live listing
            on 28 July 2026.
          </Capsule>
          <ol className="mt-4 space-y-3 text-gray-700 leading-relaxed list-decimal pl-5">
            <li>
              ...
              verified review count and rating on the live GetYourGuide listing, which is the only booking
              signal we can check independently.
            </li>
            <li>
              <span className="font-semibold text-gray-900">Then seasonal fit.</span> A tour moves up or down
              for a specific, stated reason tied to {data.month}: sea temperature, wet days, daylight hours,
              an opening-hours change or a closure. Every move is explained in the entry itself rather than
              left implied, so you can disagree with our reasoning if you want to.
            </li>
            <li>
              <span className="font-semibold text-gray-900">Genuinely seasonal products are named, not hidden.</span>{' '}
              The Costa Brava sea trip is the highest-rated tour on this site and is not bookable from about
              November to March. The Casa Batlló rooftop night visit runs 1 April to 31 August only. On the
              months where those are unavailable we put them in the closures table rather than quietly
              ranking something weaker in their place.
            </li>
            <li>
              <span className="font-semibold text-gray-900">Nothing is invented.</span> Prices, star ratings
              and review counts come from the live product page. We do not publish an opening time, closure
              or festival date we could not confirm from an official source, and where we could not confirm
              one we say so on the page.
            </li>
            <li>
              <span className="font-semibold text-gray-900">We earn a commission.</span> Booking links go to
              GetYourGuide and pay us if you book. That does not change the ranking: every review count is
              public and you can check each one against the listing.
            </li>
          </ol>

          {/* 9. FAQs */}
          <H2 id="faqs">FAQs about Barcelona in {data.month}</H2>
          ...
          <nav aria-label="Other months" className="mt-12">
            <div className="flex flex-col sm:flex-row gap-3 justify-between">
              ...
              >
                ...
                <span className="block font-semibold text-blue-900">Barcelona in {data.prevMonth}</span>
              </Link>
              <Link
                href={data.nextPath}
              ...
              >
                ...
                <span className="block font-semibold text-blue-900">Barcelona in {data.nextMonth}</span>
              </Link>
            </div>

            <div className="mt-6 rounded-lg bg-gray-50 border border-gray-200 p-4">
              <p className="text-sm font-semibold text-gray-900">Barcelona month by month</p>
...
export function buildMonthMetadata(slug: string) {
  ...
  if (!data) return {};
  const year = seasonYear(data.yearOffset);
  const title = `Barcelona in ${data.month} ${year}: Weather, Events and the Best Tours`;
  return {
